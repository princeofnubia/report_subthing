...
async function buildExcelFile(rows, stats, filePath) {
  ...
  for (let col = 1; col <= COLUMNS.length; col++) {
    headerRow.getCell(col).fill = headerFill;
  }
  ...
  for (let i = 0; i < totalRows; i++) {
    ...
    // Zebra-stripe the transaction report area: light orange / white,
    // alternating by data row (header row keeps its own styling above).
    if (dataRow) {
      const fill = {
        type: 'pattern',
        pattern: 'solid',
