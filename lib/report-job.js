...
// user_id, generated as an Excel (.xlsx) attachment. Every send also goes
// out as a separate, identical copy to each active observer email
// (BCC-style — observers don't see the original recipient or each other).
//
...
async function buildExcelFile(rows, filePath) {
...
async function sendReportForRecipient({ reportDate, userId, recipientEmail, template, observerEmails }) {
  ...
  await buildExcelFile(rows, filePath);
  ...
  for (const observerEmail of observerEmails) {
    try {
      await sendMail({
        to: observerEmail,
        subject,
      ...
      console.error(
        `Failed to send observer copy to ${observerEmail} (for user_id ${userId}):`,
        err.message
      );
    }
    await delay(300);
