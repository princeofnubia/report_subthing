...
function buildStatsTableRows(stats) {
  ...
  rows.push({ items: '', values: '' });
  rows.push({ items: 'Payment Instructions:', values: '' });
  if (PAYMENT_ACCOUNT_NAME) rows.push({ items: `Account Name: ${PAYMENT_ACCOUNT_NAME}`, values: '' });
  if (PAYMENT_BANK_NAME) rows.push({ items: `Bank: ${PAYMENT_BANK_NAME}`, values: '' });
  if (PAYMENT_ACCOUNT_NUMBER) rows.push({ items: `Account Number: ${PAYMENT_ACCOUNT_NUMBER}`, values: '' });
  rows.push({ items: `Narration: ${PAYMENT_NARRATION}`, values: '' });
